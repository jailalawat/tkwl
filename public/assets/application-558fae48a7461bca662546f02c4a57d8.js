...
(function() {
  Spree.ready(function($) {
    Spree.onAddress = function() {
      var getCountryId, order_use_shipping, update_billing_form_state;
      if (($('#checkout_form_address')).is('*')) {
        ...
        order_use_shipping.change(function() {
          return update_billing_form_state(order_use_shipping);
        });
        update_billing_form_state = function(order_use_shipping) {
...
function VariantOptions(params) {
  ...
  var allow_select_outofstock = true;
  ...
  function init() {
    ...
    if (default_instock) {
      divs.each(function(){
        $(this).find(".variant-option-values a.in-stock:first").click();
      });
    }
  }
  ...
  function disable(btns) {
    return btns.removeClass('selected active');
  }
...
//make items always in stock
        $(element).addClass(_var.in_stock ? selection.length == 1 ? 'in-stock auto-click' : 'in-stock' : 'in-stock');
...
//make items always in stock
  ...
  function toggle(variants) {
    if (variant) {
      ...
      // modified so add to cart is never disabled
  ...
  function clear(i) {
    variant = null;
    update(i);
    enable(buttons.removeClass('selected active'));
    toggle();
  ...
  function handle_click(evt) {
    ...
    disable(buttons);
    var a = enable(a.addClass('selected active'));
